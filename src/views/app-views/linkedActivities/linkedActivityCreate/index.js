...
import { Card, Select, Input, Button, Checkbox, Form, DatePicker, InputNumber } from 'antd';
...
function NewForm(props) {
    ...
    const { linkedactivityData, form, onFinish, handleAdd, handleRemove, textChanged, handleIdChange, inputData, activityListData, linkedName, nameChange } = props;
    return (
        <Form initialValues={linkedactivityData} form={form} name="control-hooks" onFinish={onFinish} style={{ marginTop: 100 }}>

            <div className="form-group row" >
                <div className="col-3" ></div>
                <div className="col-6" >
                    <Form.Item label="LinkedActivity Name" rules={[{ required: true }]}>
                        <Input style={{ width: '300px' }} defaultValue={linkedName} onChange={e => nameChange(e)} />
                    </Form.Item>
                </div>
                <div className="col-3" ></div>
            </div>
                        ...
                        {inputData.map((inputData, index) => (
                            <Fragment key={index}>
                                <div className="col-12 form-row" style={{ marginTop: 20 }} >
                                    <div className="col-4">
                                        <Input style={{ width: '250px' }} defaultValue={inputData["display_name"]} onChange={e => textChanged(index, e)} />
                                    </div>
                                    <div className="col-4">
                                        <Select style={{ width: '250px' }} defaultValue={inputData["activity_id"]} onChange={e => handleIdChange(e, index)}>
                                            {activityListData?.map((activity, index) => {
                                                return (
                                                    <Option key={index} value={activity.id}>{activity.activity_name}</Option>
                                                )
                                            })}
                                        </Select>
                                    </div>
...
function EditForm(props) {
    ...
    const editLinkedActivity = (values) => {
        dispatch(actions.editLinkedActivities(values, token)).then(() => gotoList());
    };
    ...
    useEffect(() => {
        if (linkedactivityData != null) {
            ...
            let tblData = JSON.parse(linkedactivityData.linked_activties)[0].linked_activities;
            let tblDataLst = [];
    ...
    return (
        <div>
            {(linkedactivityData && load == 1) ? (
                <NewForm linkedactivityData={linkedactivityData} activityListData={activityListData} form={form} onFinish={onFinish} handleAdd={handleAdd} handleRemove={handleRemove} textChanged={textChanged} handleIdChange={handleIdChange} inputData={inputData} linkedName={linkedName} nameChange={nameChange} />
            ) :
                (<div></div>)}
        </div>
    )
    ...
    return (
        <Card>
            ...
            <div>
                {(id == null) &&
                    <Form form={form} name="control-hooks" onFinish={onFinish} style={{ marginTop: 100 }}>
                        ...
                        <div className="form-group row" >
                            <div className="col-3" ></div>
                            <div className="col-6" >
                                <Form.Item name="name" label="LinkedActivity Name" rules={[{ required: true }]}>
                ...
                {id != 0 && linkedactivityData && load == 1 && <EditForm linkedactivityData={linkedactivityData} id={id} activityList={activityList} />}
            </div>
        </Card>
    )
}
