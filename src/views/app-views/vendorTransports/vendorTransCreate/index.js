...
function NewForm(props) {
    ...
    const { vendorTransportsData, form, onFinish, checkVal, selectChanged, selectVal, handleChange, vendorData } = props;
    return (
        <Form  {...layout} initialValues={vendorTransportsData} form={form} name="control-hooks" onFinish={onFinish}>
            ...
            <Form.Item name="transport_name" label="Transprot Name" rules={[{ required: true }]}>
                <Input />
            </Form.Item>
            <Form.Item name="oneway_price" label="Oneway Price" rules={[{ required: true }]}>
                <Input />
            </Form.Item>
            <Form.Item name="twoway_price" label="Twoway Price" rules={[{ required: true }]}>
                <Input />
            </Form.Item>
    ...
    return (
        <div>
            {(vendorTransportsData && load == 1) ? (
                <NewForm vendorTransportsData={vendorTransportsData} vendorData={vendorData} form={form} onFinish={onFinish} checkVal={checkVal} selectChanged={selectChanged} selectVal={selectVal} handleChange={handleChange} />
            ) :
                (<div></div>)}
        </div>
    )
    ...
    return (
        <Card>
            ...
            <div style={{ width: '50%', marginLeft: '17%', marginTop: 100 }}>
                ...
                {id != 0 && vendorTransportsData && load == 1 && <EditForm vendorTransportsData={vendorTransportsData} id={id} vendorData={vendorData} />}
            </div>
        </Card>
    )
}
